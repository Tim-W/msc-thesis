...
Algorithm \ref{alg:algorithm-distributed-search} is implemented using the parameters $ttl=1$ and $maxPeers=20$ so there is no recursive searching throughout the network. A low value of $ttl$ is chosen because any $ttl>1$ results in an exponential growth of search messages throughout the network.
\begin{figure}
